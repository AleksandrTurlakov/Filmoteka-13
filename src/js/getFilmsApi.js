...
export async function getFilmSearch(searchQuery, page=1) {
  console.log('searchQuery', name);
  // const url = `${BASE_URL}/search/${MEDIA_TYPE}/${TIME_WINDOW}?api_key=${API_KEY}&query=${searchQuery}&page=${page}`;
    return await axios
        .get(`${BASE_URL}/search/${MEDIA_TYPE}?api_key=${KEY}&query=${searchQuery}&page=${page}`)
        .then((response) => {
...
async function getMovieDetails(movieId) {
    const url = `${BASE_URL}/${MEDIA_TYPE}/${movieId}?api_key=${API_KEY}&language=en-US`;
    return await axios
        .get(url)
        .then((response) => {
...
async function getGenres() {
    const url = `${BASE_URL}/genre/${MEDIA_TYPE}/list?api_key=${API_KEY}&language=en-US`;
    return await axios
        .get(url)
        .then((response) => {
